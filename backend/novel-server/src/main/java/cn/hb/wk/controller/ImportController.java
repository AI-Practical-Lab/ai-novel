...
import static cn.hb.wk.pojo.CommonResult.error;
...
public class ImportController {
    ...
    public CommonResult<ImportProgressRespVO> startImportFile(@RequestParam("file") MultipartFile file,
        ...
        if (aiPlatformProperty == null || !"TONG_YI".equalsIgnoreCase(aiPlatformProperty.trim())) {
            return error(500,"当前配置的 AI 平台不支持导入功能，仅阿里百炼平台支持导入功能");
        }
        long max = 10L * 1024 * 1024;
        if (file.getSize() > max) {
            return error(500,"文件大小超过 10MB");
        }
        ...
        if (!(ext.equals("txt") || ext.equals("md") || ext.equals("docx"))) {
            return error(500, "不支持的文件类型，仅支持 txt/md/docx (当前文件: " + fileName + ")");
        }
        String content = fileParseService.parseFile(file);
        if (content != null && content.length() > 1000000) {
            return error(500, "小说总字数超过限制，最大支持 100 万字");
        }

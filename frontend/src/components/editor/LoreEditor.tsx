        ...
        <div className="h-full flex flex-col relative">
            ...
            <div className="flex items-center justify-between mb-4 shrink-0 border-b border-zinc-100 dark:border-zinc-800 pb-4">
                <div className="flex-1 mr-4">
                    <input
                        type="text"
                        className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 bg-transparent border-none outline-none hover:bg-zinc-100 dark:hover:bg-zinc-800/50 focus:bg-zinc-100 dark:focus:bg-zinc-800/50 rounded px-2 py-1 -ml-2 transition-colors w-full focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500"
                        value={lore.title || lore.name || ''}
                        placeholder="未命名设定"
                        onChange={(e) => {
                            const val = e.target.value;
                            if (lore.type === 'character' || lore.name !== undefined) {
                                onChange({ ...lore, name: val, title: val });
                            } else {
                                onChange({ ...lore, title: val });
                            }
                        }}
                    />
                    <div className="text-sm text-zinc-500 capitalize flex items-center gap-2 mt-1 px-1">

...
export default function DiffViewer({
  ...
  const goNext = () => {
    ...
    setCurrentIdx(prev => {
      ...
      if (next < diffGroups.length) {
        ...
        setTimeout(() => {
          document.getElementById(`diff-group-${next}`)?.scrollIntoView({
            behavior: 'smooth',
            block: 'center'
          });
        }, 0);
  ...
  return (
      <div className="absolute inset-0 z-10 bg-white dark:bg-zinc-900 flex flex-col animate-in fade-in duration-200">
        <div
            className="flex flex-col border-b border-zinc-200 dark:border-zinc-800 bg-purple-50/50 dark:bg-purple-900/10 shadow-sm z-20 shrink-0">
          <div className="flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-4">
              <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2">
                {title}
                <span
                    className="text-xs font-normal text-zinc-500 bg-white dark:bg-zinc-800 px-2 py-0.5 rounded-full border border-zinc-200 dark:border-zinc-700">
            ...
            <div className="flex items-center gap-3">
              <div
                  className="flex items-center gap-2 mr-4 border-r border-zinc-300 dark:border-zinc-700 pr-4">
                <button
        ...
        <div className="flex-1 overflow-auto">
          <div className="min-h-full px-6 py-4">
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
              <div
                  className="text-xs font-semibold text-zinc-600 dark:text-zinc-300 tracking-wide flex items-center justify-between">
                <span>原文</span>
                <span className="text-[10px] text-zinc-400">AI 不会改动此列内容</span>
              </div>
              <div
                  className="text-xs font-semibold text-purple-700 dark:text-purple-300 tracking-wide flex items-center justify-between">
                <span>AI 修改后</span>
                ...
                return (
                    ...
                    >
                      {group.modifiedValues.length > 0 ? group.modifiedValues.join('') : '—'}
                        ...
                        {isCurrent && (
                            <div
                                className="absolute -top-2 right-0 translate-y-[-100%] flex items-center gap-1 bg-white dark:bg-zinc-800 shadow-xl rounded-lg p-1 border border-zinc-200 dark:border-zinc-700 z-50 whitespace-nowrap">
                              <div
                                  className="px-2 py-1 text-xs font-bold text-zinc-500 uppercase border-r border-zinc-200 dark:border-zinc-700 mr-1">

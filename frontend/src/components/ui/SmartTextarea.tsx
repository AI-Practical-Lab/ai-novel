...
interface SmartTextareaProps {
    ...
    isCommandMode?: boolean; // Added mode prop
}
...
export const SmartTextarea = forwardRef<HTMLTextAreaElement, SmartTextareaProps>(({
    ...
    return (
        <div className="relative w-full">
            ...
            <div className={cn(
                "relative flex items-end gap-2 bg-white dark:bg-zinc-900 border rounded-xl p-3 shadow-sm transition-all",
                isCommandMode
                    ? "border-purple-200 dark:border-purple-800 focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-500"
        ...
        />
                ...
                <button
                    ...
                    className={cn(
                        "p-2 rounded-lg shrink-0 transition-all flex items-center justify-center",
                    ...
                    title={isCommandMode ? "发送指令" : "发送消息"}
                >
                    {isLoading ? <Square className="w-4 h-4 fill-current" /> : (isCommandMode ? <Sparkles className="w-4 h-4" /> : <Send className="w-4 h-4" />)}
                </button>
            </div>
        </div>
    );
